<template>
  <div>
    ...
    <!-- <div @click="getBaseInfo">{{didi}}</div> -->
    <el-card>
      ...
      >
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="7">
            <el-form-item label="创建时间">
              <el-input placeholder="请输入创建时间" v-model="baseInfoForm.createDate" disabled></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <!-- 第三行信息 -->
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="基地地址" label-width="120px" prop="baseAddr">
              <el-input placeholder="请输入基地地址" v-model="baseInfoForm.baseAddr" @blur="onKeyDown" clearable></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="7">
            <el-form-item label="经度">
              <el-input placeholder="请输入基地经度" v-model="baseInfoForm.basePositionLongitude" disabled></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="7">
            <el-form-item label="纬度">
              <el-input placeholder="请输入基地纬度" v-model="baseInfoForm.basePositionLatitude" disabled></el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row class="baseUpLoad">
          ...
          <el-col :span="20" class="imgFrame">
            <div v-for="(url, index) in picSets" :key="index" class="test">
              <div class="mask"><i class="el-icon-delete" @click="deleteClick(url)"></i></div>
              <img :src="url"/>
            </div>
            <el-upload
        ...
        <el-row>
          <el-button type="info" class="pullRight" @click="changeBaseInfo">保存</el-button>
        </el-row>
      </el-form>
    </el-card>
    <el-dialog :visible.sync="isShowMap" width="45%" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
  ...
  methods: {
    ...
    changeBaseInfo() {
      this.$refs.baseInfoForm.validate(async valid => {
        ...
        // 成功后，可以发起修改基地信息的网络请求
        // 调用修改基地信息的接口
        const { data: res } = await this.$http.put(`/base/${this.baseId}`, this.baseInfoForm)
        if (res.code !== 20000) {
          return this.$message.error('信息保存失败！！')
        }
    ...
    deleteClick(url) {
      $.ajax({
        url: 'http://106.75.154.40:9011/base/delPic',
        type: 'POST',
        data: {
          _method: 'DELETE',
          delUrl: url
